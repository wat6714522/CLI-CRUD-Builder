...
describe('NestJS Generator', () => {
  ...
  describe('DTO File Generation ', () => {
    it('Should correctly generate DTO Content', async () => {
      ...
      expect(paginationContent[1]).toContain('import { IsInt, IsOptional, IsPositive } from ');
  ...
  describe('Controller File Generation', () => {
    it('Should generate controller file content properly', async () => {
      ...
      expect(controllerContent[1]).toContain('export class PropertyController ');
  ...
  describe('Service File Generation', () => {
    it('Should correctly generate service file contents correctly', async () => {
      ...
      expect(serviceContent[1]).toContain('async findAll(paginationDTO: PaginationDTO)');
      expect(serviceContent[1]).toContain('async create(dto: CreateProductDTO)');
      expect(serviceContent[1]).toContain('async update(id: number, dto: UpdateProductDTO)');
    ...
    it("Should successfully make service's output directory", async () => {
      ...
      const serviceContent = fs.promises.writeFile.mock.calls[0];
      const serviceOutputPath = expect(fs.promises.mkdir).toHaveBeenCalledWith(outDirectory, {
        recursive: true,
      });
    });
  });
...
export class ProductModule {}`);
  ...
  it('Should successfully write module file ', async () => {
    ...
    expect(fs.promises.writeFile).toHaveBeenCalledWith[(outDirectory, expect.any(String), 'utf8')];
  });
